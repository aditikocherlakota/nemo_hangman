...
int gameState = INTRO;  // 0 = running, 1 = endscreen
int numFish = 6;
...
void setup()
{
  ...
  wordBank = loadStrings("desserts.txt");
...
void draw()
{
  ...
  if (gameState == INTRO)
  {
    ...
    textAlign(CENTER);
    textSize(50);
    text("NEMO WORD-GUESS", 325, 100);
    text("Press Any Key to Start", 325, 500);
    textSize(16);
    text("Instructions: Use keyboard A-Z to input letters. Guess the word to save the fish.", 325, 530);
    textSize(20);
    textAlign(LEFT);
  }
  //scale(2);
...
void keyPressed()
{
  ...
  for (int i = 0; i < word.length(); i++)
  {
    if (letter.equals(drawLetter[i][0])&&gameState == RUNNING)
    {
  ...
  if (!keyIsGood&&gameState == RUNNING)
  {
...
void mouseClicked()
{
  ...
  {
    if ((mouseX> 420 && mouseX<570 && mouseY>450 && mouseY<550)||
      (mouseX> 80 && mouseX<230 && mouseY>480 && mouseY<580))
    {
      gameState = RUNNING;
      word = wordBank[(int)random(wordBank.length)];
      drawLetter = new String[word.length()][2];
...
void drawFish()
{
  if (numFish <= 0)
  {
    ...
    background(255);
    image(Darla, 0, 0);
    text(word, (300-textWidth(word))/2, 450);
    fill(#60BDED);
    rect(80, 480, 150, 100);
    fill(0);
    text("REPLAY?", 100, 535);
...
void drawEnd()
{
  if (playerWon()) {  //checkEnd().equals(checkCorrect)) {
    ...
    if (gameState == ENDSCREEN)
    {
      ...
      rect(0, 400, 650, 600);
      fill(0);
      text(word, (445-textWidth(word))/2, 500);
      fill(#60BDED);
